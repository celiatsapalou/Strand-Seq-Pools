...
setwd("/home/celia/Downloads/Mosaicatcher/")
...
plot.SV_calls <-
  function(arg.segments,
   ...
   ####### here is where the plot starts
    ...
    #------here I need to add the individual
    ...
    if (pooled_experiment){

      counts$real_indiv=sub(".*_", "", counts$cell)
      counts <- counts[order(counts$real_indiv),]
      counts<-as.data.table(counts)

      # counts <- counts %>% rename("pool" = "sample","sample" = "real_indiv")
      # sample_cell= counts[, sample_cell := paste(sample, "-", cell)]
      # n_cells <- length(unique(counts[, sample_cell]))
   ...
   # browser()
    print('sup')
      ...
      if (!is.null(f_groups)) {
        ...
        if (nrow(groups) > 0) {
          ...
          manual_colors <-
            c(manual_colors,
              setNames(
                colorRampPalette(brewer.pal(8, "Set2"))(nrow(groups)),
                groups$group_id
              ))          # Add colors for SV classes
          # print(manual_colors)
      ...
      # print(manual_colors)
      plt <- plt +
        facet_wrap( ~ paste0(real_indiv,sample_cell), ncol = 1) +
...
# plot.SV_calls(
 ...
 # arg.complex = "simpleCalls_llr4_poppriorsTRUE_haplotagsFALSE_gtcutoff0.05_regfactor6_filterTRUE.complex.tsv",
  ...
  #arg.chromosome = "chr13",
 # arg.output = "TEST_chr13.pdf",
